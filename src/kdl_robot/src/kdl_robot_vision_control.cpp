...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            KDL::Rotation Re = KDL::Rotation::Rot(KDL::Vector(r_o[0], r_o[1], r_o[2]), aruco_angle); // rotation to align zc to s   

            // offsets definition
            Eigen::Matrix<double,3,1> offset_pos = Eigen::Matrix<double, 3,1>::Zero();
            offset_pos<<0.0,0.0,0.4;
            double offset_or=0.5;
            
            Eigen::Matrix<double,3,1> pm_base =toEigen(robot.getEEFrame().p) + toEigen(robot.getEEFrame().M)* toEigen(cam_T_object.p);//marker position wrt base frame                        
            Eigen::Matrix<double,3,1> pcd_base= pm_base +toEigen(robot.getEEFrame().M*cam_T_object.M)*offset_pos; // desired camera position wrt base frame
            ...
            Eigen::Matrix<double,3,1> e_p = computeLinearError(pcd_base,toEigen(robot.getEEFrame().p));
           
            Eigen::Matrix<double,6,1> x_tilde; x_tilde << e_p,  e_o_w[0], e_o[1], e_o[2];

            //std::cout << "xtilde: "<< std::endl << x_tilde << std::endl; //debug

            // resolved velocity control low
            Eigen::MatrixXd J_pinv = J_cam.data.completeOrthogonalDecomposition().pseudoInverse();
            dqd.data = 0.5*lambda*J_pinv*x_tilde + 10*(Eigen::Matrix<double,7,7>::Identity() - J_pinv*J_cam.data)*(qdi - toEigen(jnt_pos));
...
/*
            ...
            Eigen::Matrix<double,3,3> L1=-(1/aruco_pos.norm())*(I3-(aruco_pos_n*aruco_pos_n.transpose()));
            ...
            double k=2; 
            Eigen::Matrix<double,3,1> sd=Eigen::Matrix<double, 3, 1>::Zero();
            sd<<0.0,0.0,1.0;
            ...
            dqd.data = k*LJ_pinv*sd+10*(I7 - LJ_pinv*LJ)*(qdi - toEigen(jnt_pos));
